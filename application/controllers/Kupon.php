...
class Kupon extends CI_Controller
{
    ...
    { 
		...
		$this->form_validation->set_rules('kodekupon', 'Kode Bagian', 'required');
        if ($this->form_validation->run() == false) {
			$data['kupon'] = $this->admin->getkupon(null);
			$data['datakupon'] = $this->admin->getKodeKupon();
			$this->template->load('templates/dashboard', 'kupon/data', $data);
		}
		else{
			$input = $this->input->post(null, true);
			if ($input['kodekupon']== null){
				$data['kupon'] = $this->admin->getkupon(null);
			}
			else{
				$data['kupon'] = $this->admin->getkupon($input['kodekupon']);
			}
			$data['datakupon'] = $this->admin->getKodeKupon();
			$this->template->load('templates/dashboard', 'kupon/data', $data);
		}
	...
	public function datakupon($kondisi)
    {
		...
		$data['datakupon'] = $this->admin->getKodeKupon();
		$data['kupon'] = $this->db->get_where('data_kupon', ['sts_ambil' => $kondisi,'id_umum' => 2])->result_array();
	...
	public function generatekupon()
	{
	 ...
	 public function cetak_kuponall()
        {
			$data['kupon'] = $this->db->get_where('data_kupon', ['sts_ambil' => 'T'])->result_array();
